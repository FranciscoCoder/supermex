...
export default function Receta(props) {
  ...
  goTop();
  useEffect(() => {
    fetch(`${globalUrl}/api/recipes/${params.slug}`, {
      method: "GET",
      headers: {
  ...
  if (recipe.length > 0) {
      ...
      <section>
        <div key={recipe[0].id}>
          <div className={styleReceta.titulo}>
            <div className={styleReceta.enlaceAtras}>
              <Link to={`/${props.lang}/recetas`}>atrás</Link>
            </div>
            <h1>{recipe[0].nombre}</h1>
          </div>
          <div className={styleReceta.imagenReceta}>
            <img
              src={recipe[0].imagen}
              alt="Imagen Receta"
              width="1920px"
    ...
    return (
      <section className={styleReceta.section1}>
        <div className={styleNoEncontrado.noEncontrado}>
          No existe la receta
        </div>
      </section>
    );
